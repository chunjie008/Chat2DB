package ai.chat2db.server.web.api.controller.ai.siliconflow.listener;
...
/**
 * SiliconFlow AI SSE event source listener (OpenAI-compatible streaming).
 *
 * @author moji
 */
@Slf4j
public class SiliconFlowAIEventSourceListener extends EventSourceListener {

    private SseEmitter sseEmitter;

    private ObjectMapper mapper = new ObjectMapper().disable(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES);

    public SiliconFlowAIEventSourceListener(SseEmitter sseEmitter) {
        this.sseEmitter = sseEmitter;
    }

    @Override
    public void onOpen(EventSource eventSource, Response response) {
        log.info("SiliconFlow AI Sse connecting...");
    }

    @SneakyThrows
    @Override
    public void onEvent(EventSource eventSource, String id, String type, String data) {
        log.info("SiliconFlow AI response data：{}", data);
        if (data.equals("[DONE]")) {
            log.info("SiliconFlow AI closed");
    ...
    public void onClosed(EventSource eventSource) {
        ...
        sseEmitter.complete();
        log.info("SiliconFlow AI close sse connection...");
    }
    ...
    public void onFailure(EventSource eventSource, Throwable t, Response response) {
        try {
            ...
            if (Objects.nonNull(body)) {
                ...
                log.error("SiliconFlow AI sse response：{}", bodyString);
            } else {
                log.error("SiliconFlow AI sse response：{}，error：{}", response, t);
            }
            eventSource.cancel();
            Message message = new Message();
            message.setContent("SiliconFlow AI error：" + bodyString);
            sseEmitter.send(SseEmitter.event()
                .id("[ERROR]")
                .data(message));
            sseEmitter.send(SseEmitter.event()
            ...
            log.error("SiliconFlow AI send data error:", exception);
        }
    }
}
