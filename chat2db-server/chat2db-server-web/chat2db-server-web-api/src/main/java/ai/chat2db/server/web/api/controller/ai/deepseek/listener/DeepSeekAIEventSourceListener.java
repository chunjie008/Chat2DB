...
/**
 * DeepSeek AI SSE event source listener (OpenAI-compatible streaming).
 *
 * @author moji
 */
@Slf4j
public class DeepSeekAIEventSourceListener extends EventSourceListener {
    ...
    public void onOpen(EventSource eventSource, Response response) {
        log.info("DeepSeek AI Sse connecting...");
    }

    @SneakyThrows
    @Override
    public void onEvent(EventSource eventSource, String id, String type, String data) {
        log.info("DeepSeek AI response data：{}", data);
        if (data.equals("[DONE]")) {
            log.info("DeepSeek AI closed");
    ...
    public void onClosed(EventSource eventSource) {
        ...
        sseEmitter.complete();
        log.info("DeepSeek AI close sse connection...");
    }
    ...
    public void onFailure(EventSource eventSource, Throwable t, Response response) {
        try {
            ...
            if (Objects.nonNull(body)) {
                ...
                log.error("DeepSeek AI sse response：{}", bodyString);
            } else {
                log.error("DeepSeek AI sse response：{}，error：{}", response, t);
            }
            eventSource.cancel();
            Message message = new Message();
            ...
            log.error("DeepSeek AI send data error:", exception);
        }
    }
}
